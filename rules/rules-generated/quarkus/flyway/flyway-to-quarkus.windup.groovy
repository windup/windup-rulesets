...
ruleSet("flyway-to-quarkus-groovy")
        .addSourceTechnology(new TechnologyReference("flyway", null))
        ...
        .when(SourceMode.isDisabled(),
                ...
                .withProperty(FileModel.FILE_PATH, QueryPropertyComparisonType.REGEX, ".*/org/flywaydb/core\$"))
        .perform(new AbstractIterationOperation<FileModel>() {
            void perform(GraphRewrite event, EvaluationContext context, FileModel payload) {
                final String sourceBasePath = payload.getFilePath().replaceAll("/org/flywaydb/core\$", "")
                ...
                final String targetFolderPath = sourceBasePath +"/io/quarkus/flyway"
                ...
                ((Hint) Hint.titled("Replace the 'flyway-core' dependency with Quarkus 'quarkus-flyway' extension")
                    .withText("""A folder path related to a package from the `org.flywaydb:flyway-core` dependency has been found.  
                                    Replace the `org.flywaydb:flyway-core` dependency with the Quarkus dependency `io.quarkus:quarkus-flyway` in the application's dependencies management system (Maven, Gradle).  
                                    Further information in the link below.""")
                    .withIssueCategory(mandatoryIssueCategory)
                    .with(Link.to("Quarkus - Guide", "https://quarkus.io/guides/flyway"))
                    .withEffort(1)
                ).performParameterized(event, context, folderLocationModel)
            }
        })
        .withId("quarkus-flyway-groovy-00000")
