...
ruleSet("picocli-to-quarkus-groovy")
        .addSourceTechnology(new TechnologyReference("picocli", null))
        ...
        .when(SourceMode.isDisabled(),
                ...
                .withProperty(FileModel.FILE_PATH, QueryPropertyComparisonType.REGEX, ".*/picocli\$"))
        .perform(new AbstractIterationOperation<FileModel>() {
            void perform(GraphRewrite event, EvaluationContext context, FileModel payload) {
                final String sourceBasePath = payload.getFilePath().replaceAll("/picocli\$", "")
                ...
                final String targetFolderPath = sourceBasePath +"/io/quarkus/picocli/runtime"
                ...
                ((Hint) Hint.titled("Replace the 'picocli' dependency with Quarkus 'quarkus-picocli' extension")
                    .withText("""A folder path related to a package from the `info.picocli:picocli` dependency has been found.  
                                    Replace the `info.picocli:picocli` dependency with the Quarkus dependency `io.quarkus:quarkus-picocli` in the application's dependencies management system (Maven, Gradle).  
                                    Further information in the link below.""")
                    .withIssueCategory(mandatoryIssueCategory)
                    .with(Link.to("Quarkus - Guide", "https://quarkus.io/guides/picocli"))
                    .withEffort(1)
                ).performParameterized(event, context, folderLocationModel)
            }
        })
        .withId("quarkus-picocli-groovy-00000")
