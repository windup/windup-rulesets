...
ruleSet("agroal-to-quarkus-groovy")
        .addSourceTechnology(new TechnologyReference("agroal", null))
        ...
        .when(SourceMode.isDisabled(),
                ...
                .withProperty(FileModel.FILE_PATH, QueryPropertyComparisonType.REGEX, ".*/io/agroal/api\$"))
        .perform(new AbstractIterationOperation<FileModel>() {
            void perform(GraphRewrite event, EvaluationContext context, FileModel payload) {
                final String sourceBasePath = payload.getFilePath().replace("/io/agroal/api", "")
                ...
                final String targetFolderPath = sourceBasePath +"/io/quarkus/agroal"
                ...
                ((Hint) Hint.titled("Replace the 'agroal-api' dependency with Quarkus 'quarkus-agroal' extension")
                    .withText("""A folder path related to a package from the `io.agroal:agroal-api` dependency has been found.  
                                    Replace the `io.agroal:agroal-api` dependency with the Quarkus dependency `io.quarkus:quarkus-agroal` in the application's dependencies management system (Maven, Gradle).  
                                    Further information in the link below.""")
                    .withIssueCategory(mandatoryIssueCategory)
                    .with(Link.to("Quarkus - Guide", "https://quarkus.io/guides/datasource"))
                    .withEffort(1)
                ).performParameterized(event, context, folderLocationModel)
            }
        })
        .withId("quarkus-agroal-groovy-00000")
