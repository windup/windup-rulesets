...
ruleSet("avro-to-quarkus-groovy")
        .addSourceTechnology(new TechnologyReference("avro", null))
        ...
        .perform(new AbstractIterationOperation<FileModel>() {
            void perform(GraphRewrite event, EvaluationContext context, FileModel payload) {
                ...
                ((Hint) Hint.titled("Replace the 'avro' dependency with Quarkus 'quarkus-avro' extension")
                    .withText("""A folder path related to a package from the `org.apache.avro:avro` dependency has been found.  
                                    Replace the `org.apache.avro:avro` dependency with the Quarkus dependency `io.quarkus:quarkus-avro` in the application's dependencies management system (Maven, Gradle).  
                                    Further information in the link below.""")
                    .withIssueCategory(mandatoryIssueCategory)
                    .with(Link.to("Quarkus - Guide", "https://quarkus.io/guides/kafka"))
                    .withEffort(1)
                ).performParameterized(event, context, folderLocationModel)
            }
        })
        .withId("quarkus-avro-groovy-00000")
