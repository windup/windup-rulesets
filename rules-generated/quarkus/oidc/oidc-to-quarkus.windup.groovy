...
ruleSet("oidc-to-quarkus-groovy")
        .addSourceTechnology(new TechnologyReference("oidc", null))
        ...
        .when(SourceMode.isDisabled(),
                ...
                .withProperty(FileModel.FILE_PATH, QueryPropertyComparisonType.REGEX, ".*/org/eclipse/microprofile/auth\$"))
        .perform(new AbstractIterationOperation<FileModel>() {
            void perform(GraphRewrite event, EvaluationContext context, FileModel payload) {
                final String sourceBasePath = payload.getFilePath().replaceAll("/org/eclipse/microprofile/auth\$", "")
                ...
                final String targetFolderPath = sourceBasePath +"/io/quarkus/oidc/token/propagation"
                ...
                ((Hint) Hint.titled("Replace the 'microprofile-jwt-auth-api' dependency with Quarkus 'quarkus-oidc-token-propagation' extension")
                    .withText("""A folder path related to a package from the `org.eclipse.microprofile.jwt:microprofile-jwt-auth-api` dependency has been found.  
                                    Replace the `org.eclipse.microprofile.jwt:microprofile-jwt-auth-api` dependency with the Quarkus dependency `io.quarkus:quarkus-oidc-token-propagation` in the application's dependencies management system (Maven, Gradle).  
                                    Further information in the link below.""")
                    .withIssueCategory(mandatoryIssueCategory)
                    .with(Link.to("Quarkus - Guide", "https://quarkus.io/guides/security-openid-connect-client"))
                    .withEffort(1)
                ).performParameterized(event, context, folderLocationModel)
            }
        })
        .withId("quarkus-oidc-token-propagation-groovy-00000")
